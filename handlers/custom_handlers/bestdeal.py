    ...
    if message.text.isdigit():
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            if int(data['max_price']) < int(data['min_price']):
                ...
                bot.send_message(message.from_user.id, 'Максимальная цена не может быть меньше минимальной, повторите ввод')
                ...
                bot.send_message(message.from_user.id, 'Хорошо. Введите минимально допустимое расстояние до центра (в км)')
...
def get_min_distance_to_center(message: Message) -> None:
    """ Сохраняем диапазон расстояний до центра: минимальное """
    ...
    if message.text.isdigit():
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            bot.send_message(message.from_user.id, 'Записал. Укажите максимально допустимое расстояние до центра (в км)')
            bot.set_state(message.from_user.id, UserInfoForLowprice.max_distance_to_center, message.chat.id)
    else:
        bot.send_message(message.from_user.id, 'Пожалуйста, введите число!')
        logger.warning(f'user_id({message.from_user.id}) | не корректные данные: min_distance_to_center - {message.text}')


@bot.message_handler(state=UserInfoForLowprice.max_distance_to_center)
def get_max_distance_to_center(message: Message) -> None:
    """ Сохраняем диапазон расстояний до центра: максимальное """
    ...
    if message.text.isdigit():
        with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
            ...
            if int(data['min_distance_to_center']) > int(data['max_distance_to_center']):
                ...
                logger.info(f'user_id({message.from_user.id}) | данные приняты: max_distance_to_center - {message.text}')
                bot.set_state(message.from_user.id, UserInfoForLowprice.number_of_hotels, message.chat.id)
    else:
        bot.send_message(message.from_user.id, 'Пожалуйста, введите число!')
        logger.warning(f'user_id({message.from_user.id}) | не корректные данные: max_distance_to_center - {message.text}')
